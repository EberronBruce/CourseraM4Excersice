...
struct OurDishes: View {
    ...
    func buildPredicate() -> NSPredicate {
        if searchText.isEmpty {
            ...
            // If searchText is not empty, create a predicate for name contains searchText
            return NSPredicate(format: "name CONTAINS[cd] %@", searchText)
        }
    }
    
    // Function to build sort descriptors for sorting by name
